...
#define	FCLK_SLOW()		(SPCR = 0x52)		/* Set SPI clock for initialization (100-400kHz), F_CPU / 64 = 250kHz */
#define	FCLK_FAST()		(SPCR = 0x50)		/* Set SPI clock for read/write (20MHz max), F_CPU / 2 = 8Mhz */
...
void power_on (void)
{
	/* Run socket power on and wait for 10ms+ (nothing to do if no power controls) */
...
void power_off (void)
{
	...
	/* Run socket power off (nothing to do if no power controls) */
	//To be filled
}
